...
| `rounds` | Named question groups with optional `background_color`. `style` (`standard`/`intermission`) selects the slide expansion; `display_title` is the on-screen heading for an intermission round (falls back to `name`); `grid_columns` is its picture-grid width; `print_pictures` prints that grid on the Quizzer Answer Sheet PDF. |
...
| `QuestionManager.jsx` | Questions | Question bank CRUD, category filter, difficulty, CSV export/import (duplicate-aware), category manager. A **Kind** selector morphs the editor into a **Who/What Am I?** authoring layout (numbered clues + shared answer). A **📁 Select / upload media** button opens the shared `MediaPicker`; a selected **audio** file gets a **✂ Cut down / lyrics** button (`AudioEditor`) and a selected **video** file a **✂ Cut down / trim** button (`VideoEditor`). Each question row carries an **"⚠ In &lt;round&gt;"** label when it is already used in a round. (The legacy per-question "Format" field/filter has been removed — answer mode is set per-round in the Round builder.) |
| `RoundBuilder.jsx` | Rounds | Assemble questions into rounds, set round colour. The "Available Questions" picker in the create/edit modal filters by the same params as the Questions page — search matches **text OR answer**, plus dropdowns for category, difficulty, media type, answer mode, question format, and approval status (extra filters sit in a second `.dnd-filters-extra` row beneath search+category). Questions already used in another round carry an **"⚠ In &lt;round&gt;"** label in both the picker and the Round Order list. |
...
| `QuizControl.jsx` | Control | Live slide navigation, lock/unlock answers, lifecycle buttons, portal quick-links (lobby + active), and a **Teams panel** (edit size + ×2 power-up round, add/remove) in both lobby and live states. In **Test Quiz** mode it also renders a "Quiz Testing" banner, embedded slideshow + quizzer preview iframes, and a bot engine (`TestHarness`). |
...
| `team_updated` | Server → Clients | `{ teamId, teamName, teamSize }` | Host edited a team (size/name) on Control; scoreboards re-fetch so the handicap re-scores |
| `doubleup_chosen` | Server → Clients | `{ teamId, roundId\|null, byAdmin? }` | A team's ×2 power-up round was set (by the team or the host) or cleared (`null`) |
...
### Per-question point values (marking)
A question's `points` drives what can be awarded, so a 2-point question isn't capped at 1:
- **Auto-mark awards the full value** — `maybeAutoMark` (REST) and the `submit_answer` socket handler both read `questions.points` and insert that (a 2-pointer scores 2). Name-the-Song splits the question's value in half per part (artist / song) instead of a fixed ½.
- **Marking buttons scale** — `scoreOptions(q.points)` in AnswerMarking generates the row: half steps to 2 points (`0 / 0.5 / 1 / 1.5 / 2`), whole steps to 5, coarser above that so the row never becomes a wall of buttons. A 1-point question still shows `0 / 0.5 / 1`.
- **Free entry** — every marking row (and the Who Am I? row) also has a small number box + **Set** (`PointsInput`) to award **any** value; blank + Set clears the mark. `POST /api/answers/mark` never validated the number, so no backend change was needed; `scores.points_awarded` is `DECIMAL(3,1)`.
...
### Quizzer stickiness — reconnect + back-button guard
Getting a guest back into their team is layered so no single failure loses them (all in `frontend-quizzer/src/App.jsx`):
- **Device identity** — a client-generated UUID in `localStorage` (`qm_device_id`), sent as `deviceId` on every `/teams/join`. `GET /api/teams/session/:id/device/:deviceId` lets a device that scans the QR again drop **straight** back into its team with no form and no duplicate team.
- **Stored join identity** — `localStorage.quizTeam = { teamId, code }` (localStorage, not sessionStorage, so it survives the back button, refreshes and new tabs). On boot it re-resolves the code and re-enters the session; a stale/finished session clears it. A URL deep-link to a *different* quiz is never hijacked by the stored one.
- **Fuzzy name rejoin** — `/teams/join` find-or-creates on a normalised name; a close-but-not-exact match returns `needsConfirm` + a suggestion and the guest confirms ("Is this your team?") rather than the server guessing.
- **Pre-lobby** — a valid code scanned *before* the host starts polls every 3s and auto-joins the moment the lobby opens.
- **Back-button guard** — Back can't be disabled, so once in the quiz the join entry is `replaceState`d and a sentinel is pushed; `popstate` immediately re-pushes it and flashes "👍 You're still in the quiz" (`.back-note`). A `beforeunload` confirm covers tab close/reload while playing.
- **Connection bar** — `.conn-bar` shows an amber "⚠ Reconnecting… your answers are saved" the moment the socket drops and a green "✓ Back online" for 2.5s when it returns (nothing on first connect). Socket.io is configured with infinite retries and **buffers outgoing emits while offline**, so answers typed during an outage really do flush on reconnect (verified by stopping the backend mid-answer).
...
- **Lock-in scoring + retries**: a team guesses via the quizzer's `whoami_clue` slide ("Lock In"). `POST /api/whoami/lock { sessionId, teamId, clueIndex, guess }` — the server looks up the clue's points from the widget config (the client only sends the index) and auto-marks: correct → clue points and `locked = true` (immutable), **wrong → 0 points but the row stays UNLOCKED**, so the team gets another go on the **next** clue for that clue's lower value. **One attempt per clue** (a repeat submission on the same `locked_clue_index` returns 409), so nobody can brute-force a single clue. Guessing early is therefore rewarded but no longer risks locking a team out for the whole quiz.
  The quizzer tracks this as `whoamiMiss` ({ clueIndex, guess }) separately from `whoamiLock`: on the spent clue it shows "Not this time — … you get another go on the next clue", and on the next clue a "🔄 New clue, new go" banner above a fresh form. Both states restore after a refresh/rejoin from `GET /api/whoami/session/:id` (a row with `locked = false` restores as a miss). `whoami_locked` is only broadcast for a **correct** guess, and the quizzer ignores `autoMarked` `whoami_marked` events for creating a lock (otherwise a wrong guess's 0 would re-lock the form).
- **Marking**: AnswerMarking shows a Who Am I? section per team (guess, clue locked on, **0** / **full clue points** buttons, plus a free-entry box to type **any** number of points). An admin mark also sets `locked = true` so a later team guess can't overwrite the host's decision; clearing the mark leaves the lock as it was. All via `POST /api/whoami/mark`.
...
### Double Up (per-team ×2 power-up) + editing teams from Control
Each team may nominate **one round to score ×2**. It's per-team, not global: teams pick on the quizzer's **Double Points** page (a `doubleup` widget dropped in the quiz order), stored one row per team in `double_up_choices` (UNIQUE on `team_id`). The ×2 is applied at **scoreboard-aggregation time** (`utils/doubleUp.js`), so raw `scores` rows are never altered and the multiplier is instantly reversible. Intermission (picture) rounds are excluded from the picker.
...
**Reachable from the Question editor too:** a selected **audio** file shows **✂ Cut down / lyrics** (`AudioEditor`) and a selected **video** file shows **✂ Cut down / trim** (`VideoEditor`) on the media row in `QuestionManager`, so a clip can be cut without leaving the question. `currentMediaFile()` resolves the selection to the full `media_files` record (via `mediaByUrl`) — that's what supplies the `id`, so **♻ Overwrite original** is offered for library files. On save, `handleAudioEdited` / `handleVideoEdited` point the question at whatever came back (a new file's url, or the same url when overwritten). Video has **no subtitle/lyrics layer** — trimming is the whole editing story for video.

**ffmpeg.wasm core delivery (no CDN):** the `ffmpegCore()` plugin in `frontend-admin/vite.config.js` copies the core + wasm to a stable served path **`/ffmpeg/ffmpeg-core.{js,wasm}`** — emitted into the production build (served by the admin nginx) and via dev middleware locally. `VideoEditor` loads them with `toBlobURL` from `import.meta.env.BASE_URL + 'ffmpeg'`, so it works offline behind any host. Single-thread core needs **no** COOP/COEP cross-origin-isolation headers. `optimizeDeps.exclude: ['@ffmpeg/ffmpeg','@ffmpeg/util']` keeps Vite from rewriting the worker paths. New admin deps: `@breezystack/lamejs`, `@ffmpeg/ffmpeg`, `@ffmpeg/util`, `@ffmpeg/core`.

**It must be the ESM core — `dist/esm`, not `dist/umd`.** `@ffmpeg/ffmpeg` 0.12.15 creates its worker with `type: "module"` on **both** branches of `load()` (`classes.js`), and `importScripts()` does not exist in a module worker. So the worker's `try { importScripts(coreURL) }` always throws and the fallback `self.createFFmpegCore = (await import(coreURL)).default` is the only path that runs. A **UMD** core imported as a module has no default export → `createFFmpegCore` is undefined → the worker throws `ERROR_IMPORT_FAILURE`, surfaced in the UI as **"Trim failed: Error: failed to import ffmpeg-core.js"**. `require.resolve('@ffmpeg/core')` honours the `require` condition and returns the **umd** path, so the plugin deliberately steps across to `dist/esm` (`join(dirname(require.resolve('@ffmpeg/core')), '..', 'esm')`). If a future bump breaks video trimming with that error, check which build is being served: the ESM core starts `var _scriptDir = import.meta.url`, the UMD one `document.currentScript`.

**Unsupported source formats:** trimming reads the clip's duration from a hidden `<video>`, so the file has to be decodable *by the browser* (H.264/VP8/VP9 MP4/WebM). HEVC, MPEG-4 Part 2 and `.mkv` fail to load and both Save buttons stay disabled — `VideoEditor`'s `onError` explains this instead of leaving a dead dialog.
...
- **Round Builder** has a **Round style** dropdown; choosing Intermission reveals a **display-title** field, a **grid-columns** selector (3/4/5) with a live square-layout preview, and a **🖨 Print pictures on the answer sheet** tick (`rounds.print_pictures`) that makes the Quizzer Answer Sheet PDF print this round's picture grid. The picker also has a **sort** control (Default/A→Z/Category/Newest) to cluster a picture set together (filter media type = image + a shared category). Fields persist via `roundController` create/update and flow through `loadQuizWithRoundsAndWidgets` (`r.*`).
...
- **Quizzer Answer Sheet** (PDF) — one page per round, **boxes only — the question text is never printed** (teams read questions off the screen). Numbered write-in boxes; MCQ questions get lettered tick-boxes (A/B/C…, no option text). Row height is divided across the questions so a round always fits **exactly one page**. Team name/size on page 1, **no answers**. A **join QR + URL is printed top-right of every page** (`drawJoinQr`) so a team can rejoin from whichever sheet is in front of them — the link is `<quizzerUrl>/<quiz code>`, resolved from `GET /api/config` with the usual `hostname:3003` fallback. A **Who Am I? guess area prints on every page that has a clue** (`drawWhoamiBox`): round *i* pairs with clue *i* exactly as `buildSlides` distributes them, and the box is labelled with the clue number and its point value. An intermission round whose **`print_pictures`** is on prints its **picture grid** instead of plain boxes (`drawPictureGrid`): numbered thumbnails, each with its own write-in box, scaled to fit one page; a picture that fails to load prints "image unavailable" rather than a silent blank frame. `print_pictures` **defaults ON** for newly created picture rounds (`EMPTY` in RoundBuilder) — rounds created before the column existed default to `FALSE` in the DB and need the tick (open the round and save). `downloadAnswerSheet` is **async** for this (images and the QR are fetched first).
...
### "Already in a round" label (Round Builder + Question bank)
Both pages fetch `GET /rounds` (which aggregates each round's `round_questions`) and build a `questionUsage` map (`questionId → [{ id, name }]`). An orange **`.rq-used-label`** badge — `⚠ In <round name>` for one round, `⚠ In N rounds` for several, full list in the `title` tooltip — is rendered by `UsedBadge` (RoundBuilder) / `UsedInRoundsBadge` (QuestionManager):
- **RoundBuilder** shows it in the *Available Questions* picker **and** the *Round Order* list, always excluding the round currently being edited (`usedIn(q)` filters on `editingId`) — so a question only in this round shows no badge.
- **QuestionManager** shows it on every question row in the bank list.
...
Question **video never plays on a phone at all** and audio never sounds there unprompted — the big screen is the only place media runs. On the quizzer a video question renders a **still, silent frame** (`.media-noplay`: `muted`, **no `controls`**, `pointer-events: none`) with a "🎬 Playing on the main screen — tap to enlarge" caption, so no team can scrub, pause or re-watch ahead of the room; audio questions keep their explicit tap-to-play button (`QuestionAudioPlayer`, still cut off at `audioStop` for finish-the-lyrics).

**Tap-to-enlarge lightbox (quizzer):** picture-round thumbnails, question images, answer-reveal pictures and the video frame are all tappable (`.is-zoomable`) and open an **in-page** overlay (`.media-zoom` — fixed, `z-index: 1200`) with a large view, a label, a × button and tap-anywhere-to-close. It never navigates, so a team keeps their place and their typed answers (verified). Video in the lightbox stays muted and control-less too. On the slideshow, audio/video render **without native controls**; a `QuestionMedia` component (keyed by slide index, so it unmounts/stops when you leave the slide) plays only when a `media_play` socket signal targets the current slide, and honours the `finish_the_lyrics` `audioStop`. Audio shows a non-interactive ♪/🔊 status badge.
...
- **Vite dev servers are host-only**: the containers always serve the **nginx production build**, so `server.proxy` in each `vite.config.js` is only used when you run `npm run dev` on the host (as `.claude/launch.json` does). All three now proxy `/api`, `/socket.io` and `/uploads` to **`localhost:5000`** — the quizzer/slideshow used to target `http://backend:5000`, a Docker-network name that never resolves from the host, and none proxied `/uploads`, so the SPA fallback answered image requests with `index.html` and every uploaded picture silently failed to render in dev. Reaching the backend from a host dev server still needs its port published (uncomment the `ports:` block on the `backend` service).
