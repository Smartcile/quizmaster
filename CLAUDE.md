...
- **Quizzer Answer Sheet** (PDF) — one page per round, **boxes only — the question text is never printed** (teams read questions off the screen). Numbered write-in boxes; MCQ questions get lettered tick-boxes (A/B/C…, no option text). Row height is divided across the questions so a round always fits **exactly one page**. Team name/size on page 1, a Who-Am-I guess box if present, **no answers**. A **join QR + URL is printed top-right of every page** (`drawJoinQr`) so a team can rejoin from whichever sheet is in front of them — the link is `<quizzerUrl>/<quiz code>`, resolved from `GET /api/config` with the usual `hostname:3003` fallback. An intermission round whose **`print_pictures`** is on prints its **picture grid** instead of plain boxes (`drawPictureGrid`): numbered thumbnails, each with its own write-in box, scaled to fit one page. `downloadAnswerSheet` is **async** for this (images and the QR are fetched first).
