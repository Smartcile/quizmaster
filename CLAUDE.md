...
The legacy "Format" field is gone from the Question editor and all search filters. Instead, each question row in the **Round builder** has a **Text / MCQ / Both** selector — but **only for questions that actually have options** (`hasOpt(q)`); options-less questions show a static **Text** badge and cannot be set to MCQ/Both. The override is stored as `round_questions.question_format_override`. A yellow **"🔀 T&M"** (Text & Multichoice) availability badge appears on every question that has options — in both the Available Questions list and the Round Order list — signalling that MCQ is selectable for it (this replaced the old "BOTH"-only badge). `loadQuizWithRoundsAndWidgets` maps the override onto the effective `answer_mode` (`standard→text`, `multichoice→mcq`, `both→both`) so the quizzer renders the chosen input style for that round.
